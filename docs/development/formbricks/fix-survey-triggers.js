...
/**
 * Fix survey triggers by recreating surveys with proper action class references
 * This script recreates the three widget surveys with correct trigger format
 */

/**
 * Create surveys with proper trigger references using action class keys
 */

const surveys = [
  {
    name: "Feature Request Feedback - Fixed Triggers",
  ...
  {
    name: "Bug Report Feedback - Fixed Triggers",
    environmentId: "ENVIRONMENT_ID_PLACEHOLDER",
    type: "app",
    status: "inProgress",
    welcomeCard: {
  ...
  {
    name: "Love LawnQuote Feedback - Fixed Triggers",
    environmentId: "ENVIRONMENT_ID_PLACEHOLDER",
    type: "app",
    status: "inProgress",
    welcomeCard: {
...
async function fixSurveyTriggers() {
  ...
  console.log('🔧 Creating fixed surveys with proper triggers...\n');
  ...
  // Verify fixed surveys
  console.log('🔍 Verifying fixed surveys...\n');
  ...
  try {
    ...
    if (surveys.success) {
      const fixedSurveys = surveys.data.filter(s => s.name.includes('Fixed Triggers'));
      console.log(`✅ Found ${fixedSurveys.length} fixed surveys`);
      
      fixedSurveys.forEach(survey => {
